...
class _LeaveRequestListScreenState extends State<LeaveRequestListScreen> {
  ...
  List<Map<String, dynamic>> get _filteredRequests {
    List<Map<String, dynamic>> filtered = _requests.where((req) {
      ...
      if (_fromDate != null || _toDate != null) {
        final requestDate = req['start_date'] != null ? DateTime.parse(req['start_date']) : null;
    ...
    if (_sortBy == 'time_desc') {
      ...
      filtered.sort((a, b) => (a['approved_by']?['name'] ?? '').toString().compareTo((b['approved_by']?['name'] ?? '').toString()));
    }
    return filtered;
  }
  ...
  Future<void> _cancelRequest(String leaveId) async {
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    final user = userProvider.user;
    if (user == null) return;
    showDialog(
    ...
    try {
      await ApiService().cancelLeaveRequest(
        context: context,
        apiToken: user.data.apiToken,
        leaveId: leaveId,
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: _scaffoldKey,
      backgroundColor: Colors.white,
      appBar:  widget.isAllUsers ? CustomAppBar(
